...
module.exports = async (bill) => {
  ...
  new Date();
  console.log(bill.orderProducts);
  var data = {
    ...
    products: bill.orderProducts.map(({ unitPrice, quantity, productId }) => {
      return {
        quantity,
  ...
  try {
    await fs.access("invoices", constants.F_OK);
  } catch (err) {
    await fs.mkdir("invoices", { recursive: true });
  }
  await fs.writeFile(`invoices/${bill.id}.pdf`, file.pdf, "base64");
